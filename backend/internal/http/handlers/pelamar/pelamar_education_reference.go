...
func persistCustomEducationReferences(db *sqlx.DB, userID int64, educations []map[string]any) error {
	...
	for _, education := range educations {
		institution := strings.Join(strings.Fields(strings.TrimSpace(handlers.AnyToString(education["institution"]))), " ")
		program := strings.Join(strings.Fields(strings.TrimSpace(handlers.AnyToString(education["field_of_study"]))), " ")
